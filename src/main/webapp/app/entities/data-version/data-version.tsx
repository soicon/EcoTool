...
export interface IDataVersionProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type IDataVersionState = IPaginationBaseState;

export class DataVersion extends React.Component<IDataVersionProps, IDataVersionState> {
  state: IDataVersionState = {
  ...
  render() {
    const { dataVersionList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="data-version-heading">
          Data Versions
        ...
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {dataVersionList.map((dataVersion, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${dataVersion.id}`} color="link" size="sm">
                      {dataVersion.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${dataVersion.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${dataVersion.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ dataVersion }: IRootState) => ({
  dataVersionList: dataVersion.entities,
  totalItems: dataVersion.totalItems
});
