...
export class FileStatus extends React.Component<IFileStatusProps, IFileStatusState> {
  ...
  render() {
    const { fileStatusList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="file-status-heading">
          File Statuses
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={this.sort('download_result_url')}>
                  Download Result Url <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {fileStatusList.map((fileStatus, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${fileStatus.id}`} color="link" size="sm">
                      {fileStatus.id}
                    </Button>
                  </td>
                  ...
                  <td>{fileStatus.download_result_url}</td>
                  <td>{fileStatus.fileType}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${fileStatus.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${fileStatus.id}/edit`} color="primary" size="sm">
